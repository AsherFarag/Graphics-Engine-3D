...
void CircleRB::Draw(float a_Alpha)
{
    ...
    aie::Gizmos::add2DCircle(m_SmoothedPosition, GetRadius(), m_DrawSegments, m_Colour);
    aie::Gizmos::add2DLine(GetPosition(), GetPosition() + GetNormal() * GetRadius() * vec2(-1, 1), vec4(0, 1, 0, 1));
}
