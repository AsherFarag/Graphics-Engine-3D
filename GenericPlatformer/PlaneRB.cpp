...
#pragma region Friction Calculation
	...
	float FrictionForceMagnitude = -a_RigidBody->GetDynamicFrictionCoefficient() * length(RelativeVelocity);

	// Calculate the maximum friction force magnitude based on the normal force
	float MaxFrictionForceMagnitude = a_RigidBody->GetStaticFrictionCoefficient() * ResultForce.length();
	...
	vec2 FrictionForce = FrictionForceMagnitude * RelativeVelocity;

	a_RigidBody->ApplyForce(FrictionForce, a_Contact - a_RigidBody->GetPosition());
