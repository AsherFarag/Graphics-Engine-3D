...
void PhysicsManager::Update(float DeltaTime)
{
    ...
    while (AccumulatedTime >= m_TimeStep)
    {
        ...
        for (int Outer = 0; Outer < ActorCount - 1; Outer++)
        {
            ...
            {
                // Hotfix for when rigid bodies are deleted
                if (Outer >= m_RigidBodies.size() || Inner >= m_RigidBodies.size())
                    continue;

                RigidBody* object1 = m_RigidBodies[Inner];
                RigidBody* object2 = m_RigidBodies[Outer];
...
void PhysicsManager::Draw(float a_Aplha)
{
    // Iterate backwards to handle errors when objects are deleted
    for (int i = m_PhysicsObjects.size() - 1; i >= 0; --i)
    {
        m_PhysicsObjects[i]->Draw(a_Aplha);
    }
}
...
bool PhysicsManager::PlaneToBox(RigidBody* a_Plane, RigidBody* a_Box)
{
    ...
    if (a_Plane == nullptr || a_Box == nullptr)
        return false;
    ...
    if (NumContacts > 0)
    {
        Plane->ResolveCollision(Box, Contact * (1.f / (float)NumContacts));
        return true;
    }
