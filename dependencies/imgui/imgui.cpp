...
IMGUI_API bool ImGui::DragFloatXYZ(bool IsColour, const char* label, float v[3], float v_speed, float v_min, float v_max, const char* display_format, float power)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        PushID(i);
        value_changed |= DragFloat(Label, &v[i], v_speed, v_min, v_max, display_format, power);
        SameLine(0, g.Style.ItemInnerSpacing.x);
        PopID();
        PopItemWidth();
        PopStyleColor();
