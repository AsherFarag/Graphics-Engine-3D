...
#define DEFAULT_LINEAR_DRAG vec2(0.3f)  // Between 0 and 1
...
public:
    ...
    void ApplyVelocity(vec2 a_Velocity);
    void ApplyForce(vec2 a_Force, vec2 a_ImpulsePosition = vec2());
...
public:
    ...
    vec2  GetPosition() { return m_Owner->GetPosition(); }
    void  SetPosition(vec2 a_NewPosition = vec2()) { m_Owner->SetPosition(a_NewPosition); }

    vec2 ToWorld(vec2 a_Position, float a_Alpha) { return GetPosition() + GetLocalX() * a_Position.x + GetLocalY() * a_Position.y; }

    float GetRotation() { return m_Owner->GetRotation(); }
    void  GetRotation(float a_NewRotation) { m_Owner->SetRotation(a_NewRotation); }
